...
static void BM_AES_128_ECB(benchmark::State& state)
{
    const std::array<uint8_t, aesni_key_size_128> key = {
        0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b,
        0x0c, 0x0d, 0x0e, 0x0f
    ...
    {
        aesni_key_expansion_128(round_keys.data(), key.data());
        aesni_encrypt_blocks_128(output_blocks.data(), input_blocks.data(),
                round_keys.data(), input_blocks.size() / aesni_block_size);
    }
    state.SetBytesProcessed(int64_t(state.iterations()) *
            int64_t(state.range(0)));
}
BENCHMARK(BM_AES_128_ECB)->RangeMultiplier(4)->Range(16, 1 << 14);
